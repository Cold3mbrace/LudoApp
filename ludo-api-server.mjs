...
async function readTickets() {
  const payload = await readJson(TICKETS_PATH, []);
...
function extractReadableTextFromHtml(html = "") {
  ...
  const paragraphMatches = [...String(articleMatch).matchAll(/<(?:p|li|h1|h2|h3|blockquote)[^>]*>([\s\S]*?)<\/(?:p|li|h1|h2|h3|blockquote)>/gi)]
    .map((match) => stripHtml(match[1] || ""))
    .map((chunk) => chunk.trim())
    .filter(Boolean);

  const merged = paragraphMatches.join("\n\n").trim();
  return merged || stripHtml(articleMatch || html || "");
}

async function fetchArticleBody(url) {
  const cleanUrl = String(url || "").trim();
  if (!/^https?:\/\//i.test(cleanUrl)) {
    throw new Error("Передай полный URL статьи.");
  }
  ...
  const description = decodeHtml(extractMetaContent(html, "og:description") || extractMetaContent(html, "description") || "");
  const body = extractReadableTextFromHtml(html);
  const compactBody = String(body || "").replace(/\n{3,}/g, "\n\n").trim();

  return {
    title,
    body: compactBody.length >= 180 ? compactBody : (compactBody || description || title || ""),
    description,
  };
}
...
app.get("/api/translate", async (req, res) => {
  try {
    ...
    if (!text) throw new Error("Передай text");
    const translated = await translateTextToRu(text);
    res.json({ ok: true, translated: translated || text });
  } catch (error) {
    res.status(400).json({
...
app.get("/api/news/article", async (req, res) => {
  try {
    ...
    res.json({ ok: true, ...article, url });
  } catch (error) {
    res.status(400).json({
      error: error instanceof Error ? error.message : "Не удалось раскрыть статью.",
    });
  }
});
...
app.post("/api/tickets", async (req, res) => {
  try {
    ...
    if (!message) throw new Error("Опиши проблему.");

    const key = String(req.body?.key || "guest").trim() || "guest";
    const currentUser = await readUserState(key);
    const reporterUserId = Number(req.body?.reporterUserId || currentUser?.telegram?.id || 0) || null;
    const reporterName =
      String(req.body?.reporterName || "").trim() ||
      telegramIdentityView(currentUser?.telegram || {}).name ||
      currentUser?.steam?.personaname ||
      "Пользователь";
    const reporterHandle =
      String(req.body?.reporterHandle || "").trim() ||
      (currentUser?.telegram?.username ? `@${currentUser.telegram.username}` : "");

    const ticket = {
      id: `ticket_${Date.now()}_${crypto.randomBytes(3).toString("hex")}`,
      status: "open",
      createdAt: Date.now(),
      updatedAt: Date.now(),
      message,
      source: {
        type: String(req.body?.sourceType || "unknown").trim() || "unknown",
        id: String(req.body?.sourceId || "").trim() || null,
        title: String(req.body?.title || "Сообщение о проблеме").trim(),
        url: String(req.body?.url || "").trim() || null,
      },
      reporter: {
        name: reporterName,
        handle: reporterHandle || null,
...
app.post("/api/tickets/:id/status", async (req, res) => {
  try {
    ...
    if (!ticketId) throw new Error("Передай id тикета.");

    const status = normalizeTicketStatus(req.body?.status);
    const items = await readTickets();
    const index = items.findIndex((ticket) => String(ticket?.id || "") === ticketId);
    if (index === -1) {
    ...
    res.status(400).json({
      error: error instanceof Error ? error.message : "Не удалось обновить статус тикета.",
    });
  }
});
